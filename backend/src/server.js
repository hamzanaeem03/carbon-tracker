...
const MONGO_URI =
  process.env.MONGO_URI || "mongodb://localhost:27017/carbon_dev";
const app = express();
...
app.post("/api/activities", async (req, res) => {
  ...
  try {
    ...
    metrics.totalCarbon.set(
      await Activity.aggregate([
        { $group: { _id: null, sum: { $sum: "$co2Kg" } } },
      ]).then((r) => (r[0] ? r[0].sum : 0))
    );
    metrics.activeUsers.set(
      await Activity.distinct("userId").then((list) => list.length)
    );
...
app.get("/api/global-stats", async (req, res) => {
  ...
  try {
    const totalUsers = await Activity.distinct("userId").then(
      (list) => list.length
    );
    const agg = await Activity.aggregate([
      {
        $group: { _id: null, totalCO2: { $sum: "$co2Kg" }, count: { $sum: 1 } },
      },
    ]);
